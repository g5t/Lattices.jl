...
function absorption_correction(tθ::T,s::ShapeSample;kwds...) where T
    warn("ShapeSample (with only the second moments of the sample distribution)\n is insufficient to calculate an absorption correction. Use GeometrySample instead.")
    return one(T)
end
type GeometrySample{T} <: Sample{T}
    # A sample (can be) comprised of a single crystal, its orienting vectors, its shape, and its scattering sense.
    # Two samples with all fields equivalent are not necessarily the same sample, so type Sample is not immutable.
    # It might be useful to include additional sample parameters here
    crystal::Crystal{T}
    ...
    y::Lattice3Vector{T}
    z::Lattice3Vector{T}
    geometry::SampleMesh{T}
    μ::T # the inverse absoprtion length from, e.g., NIST
    shape::Matrix{SecondMoment{T}}
    mosaic::Array{T,1} # horizontal and vertical mosaic FWHM in radians
    sense::Int8 # scattering sense, +1 or -1 -- could easily be a Bool, but as Int8 the sign is maintained if someone accesses Sample.sense in a calculation
end
...
Initialize a `ShapeSample` object after creating an orthonormal basis from `o1` and `o2`, and converting
...
function Sample{T<:Real}(c::Crystal{T},
    ...
    s=ShapeSample{T}(c,orthonormalize(orient1,orient2)...,shape,mosaic,Int8(signbit(sense)?-1:1))#,sG,sGNo)
end
"""
    Sample(c::Crystal,o1::Vector,o2::Vector,geometry::SampleMesh,mosaic::Vector,sense::Real;kwds...)
Initialize a `GeomtrySample` object after creating an orthonormal basis from `o1` and `o2`, and converting
`sense` to a `Int8` with only its sign preserved. The slurped keywords are passed to `determine_second_moments`
which calculates the second moments of the passed geometry.
"""
function Sample{T<:Real}(c::Crystal{T},o1::Vector,o2::Vector,geometry::SampleMesh,
                         mosaic::Vector{T}=[1,1]/10800*pi,sense::Real=1,inverse_absorption_length_mm=0;kwds...)
    #shape=convert(Matrix{SecondMoment{T}},determine_second_moments(geometry;kwds...))
    shape=determine_second_moments(geometry;kwds...)
    orient1=Lattice3Vector(convert.(T,o1),c.rlat)::Lattice3Vector{T} # Orienting vectors are typically
    orient2=Lattice3Vector(convert.(T,o2),c.rlat)::Lattice3Vector{T} # defined in reciprocal lattice units
    s=GeometrySample{T}(c,orthonormalize(orient1,orient2)...,geometry,inverse_absorption_length_mm,shape,mosaic,Int8(signbit(sense)?-1:1))#,sG,sGNo)
end
"""
    Sample(c::Crystal,o1::Vector,o2::Vector,o3::Vector,shape::Matrix{SecondMoment},mosaic::Vector,sense::Real)
Initialize a `ShapeSample` object after creating an orthonormal basis from `o1`,`o2` and `o3`, and converting
`sense` to a `Int8` with only its sign preserved.
"""
function Sample{T<:Real}(c::Crystal{T},o1::Vector,o2::Vector,o3::Vector,
    ...
    s=ShapeSample{T}(c,orthonormalize(orient1,orient2,orient3)...,shape,mosaic,Int8(signbit(sense)?-1:1))#,sG,sGNo)
end
"""
    Sample(c::Crystal,o1::Vector,o2::Vector,o3::Vector,geometry::SampleMesh,mosaic::Vector,sense::Real)
Initialize a `GeometrySample` object after creating an orthonormal basis from `o1`,`o2` and `o3`, and converting
`sense` to a `Int8` with only its sign preserved. The slurped keywords are passed to `determine_second_moments`
which calculates the second moments of the passed geometry.
"""
function Sample{T<:Real}(c::Crystal{T},o1::Vector,o2::Vector,o3::Vector,
                         geometry::SampleMesh,
                         mosaic::Vector{T}=T[1,1]/10800*pi,
                         sense::Real=1,inverse_absorption_length_mm=0;kwds...)
    orient1=Lattice3Vector(convert.(T,o1),c.rlat)::Lattice3Vector{T} # Orienting vectors are typically
    orient2=Lattice3Vector(convert.(T,o2),c.rlat)::Lattice3Vector{T} # defined in reciprocal lattice units
    orient3=Lattice3Vector(convert.(T,o3),c.rlat)::Lattice3Vector{T}
    #shape=convert(Matrix{SecondMoment{T}},determine_second_moments(geometry;kwds...))
    shape=determine_second_moments(geometry;kwds...)
    s=GeometrySample{T}(c,orthonormalize(orient1,orient2,orient3)...,geometry,inverse_absorption_length_mm,shape,mosaic,Int8(signbit(sense)?-1:1))#,sG,sGNo)
end
...
getgeometry(s::GeometrySample)=s.geometry
getgeometry(s::ShapeSample)=throw(TypeError)
